...
namespace DV_Station_SCADA_Series
{
    ...
    class RecreateHistorical
    {
        ...
        public void RecreatePast(string aOutputFilename, int aPastYears, int aFutureYears, DataTable aDataTable, 
            ...
            )
        {
            ...
            do
            {
                ...
                System.Diagnostics.Debug.WriteLine("DatasetPacket.Index " + DatasetPacketBookmark.Index.ToString("00") + ", " + iCounter.ToString("0000000"));
                ...
                double noiseTotal = 0;
                int iStartBinIndex = DatasetPacketBookmark.IndexStart + 1;
                ...
                for (int i = DatasetPacketBookmark.IndexStart + 1; i < DatasetPacketBookmark.IndexEnd; i++)  // + 1
                {
                    ...
                    // Ensure time for next BIN does not create pressure surge due to short dTime
                    // DanH 2017/09/17
                    if (i == iStartBinIndex)
                    {
                        // secDelta = Constant.NEXT_BIN_DELAY_SECONDS;
                        secDelta = PacketCurrentDate.Subtract(PacketPreviousDate).TotalSeconds;

                        // Add a delay  DanH 2017/09/17
                        if (secDelta < Constant.NEXT_BIN_DELAY_SECONDS)
                            secDelta = Constant.NEXT_BIN_DELAY_SECONDS;
                        else if (secDelta > Constant.MAX_BIN_DELAY_SECONDS)
                            secDelta = Constant.MAX_BIN_DELAY_SECONDS;
                    }
                    else
                        secDelta = PacketCurrentDate.Subtract(PacketPreviousDate).TotalSeconds;
                    ...
                    value = System.Convert.ToDouble(dataTable.Rows[i][Constant.FLD_VALUE]);

                    //if (value > 160)
                    //    System.Diagnostics.Debug.WriteLine(iCounter.ToString("00") + ", " + value.ToString("000"));
                    ...
                    if (iCounter >= aFactorSmooth)
                    {
                        ...
                        smoothNoise = noiseTotal / (double)aFactorNoise;  
                        smoothNoise = smoothNoise / (double)aFactorSmooth;  // Moving Average
                    ...
                    // Diagnostics
//                    if ((iCounter > 40000) && (iCounter < 50000))
                        //if (value > 150) 
                        //    System.Diagnostics.Debug.WriteLine(i.ToString("000") + "\t" + secDelta.ToString("0.0") + "\t" +
                        //    LogDateTime.ToString() + "\t" + value.ToString("0.0") + "\t" + HistoricalRecord.DataSetGroup.ToString() + "\t" +
                        //    valueNoisy.ToString("0.0"));

                    iCounter++;
                    if (iCounter > 1E8)
                    {
                        System.Diagnostics.Debug.Assert(false, "Loop > 1E8");
                        // throw DivideByZeroException;  // Circuit Breaker
...
//                    DateTime.Now.ToString("yyyy-MM-dd-HH-mm") + ".BIN";
            ...
            // TODO:  Serialize to CSV  DanH 2017/09/18
            SerializeTo(aOutputFilename, HistoricalRecordList);
            ...
            StatHisto.SaveToFile(aOutputFilename + ".HISTOGRAM.CSV", aOutputFilename + ", processed in " + timeDiff.ToString("00") + " s", false);
            ...
            StatHisto.SaveToFile(aOutputFilename + ".HISTOGRAM_WITH_NOISE.CSV", aOutputFilename + ", processed in " + timeDiff.ToString("00") + " s", false);
